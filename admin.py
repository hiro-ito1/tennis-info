...
def add_weekday(date_str):
    try:
        ...
        day = event_date.day
        return f"{month}/{day}({WEEKDAY_JP[event_date.weekday()]})"
    except Exception:
        return date_str
...
def bulk_toggle_cat():
    ...
    save_events(events)
    rebuild_html()
    return redirect(url_for("index", msg=f"{count}件のカテゴリを変更しました"))
...
def add_manual():
    ...
    new_event = {
        "id": f"manual_{datetime.now().strftime('%Y%m%d%H%M%S')}_{re.sub(r'[^\w]', '_', title[:10])}",
        "status": "approved",
